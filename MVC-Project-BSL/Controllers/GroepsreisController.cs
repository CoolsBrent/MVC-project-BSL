...
namespace MVC_Project_BSL.Controllers
{
    ...
    public class GroepsreisController : Controller
    {
        ...
        {
            // Haal alle monitoren op, inclusief de gerelateerde Persoon-data, 
            // waarbij alleen actieve personen worden opgehaald
            var monitoren = await _unitOfWork.MonitorRepository.GetAllAsync(
                query => query.Include(m => m.Persoon)
                              .Where(m => m.Persoon.IsActief));

            // Haal alle deelnemers op, inclusief de gerelateerde Persoon-data
            var deelnemers = await _unitOfWork.KindRepository.GetAllAsync(
                query => query.Include(m => m.Persoon));

            // Haal de specifieke Groepsreis op met alle relevante data
            var groepsreis = await _unitOfWork.GroepsreisRepository.GetQueryable()
                .Include(g => g.Monitoren)
                    .ThenInclude(m => m.Monitor.Persoon) // Voeg Monitor en gerelateerde Persoon-data toe
                .Include(g => g.Bestemming)
                    .ThenInclude(b => b.Fotos) // Voeg Bestemming en gerelateerde foto's toe
                .Include(g => g.Deelnemers)
                .Include(g => g.Programmas)
                    .ThenInclude(p => p.Activiteit) // Voeg Programma en gerelateerde Activiteit toe
                .FirstOrDefaultAsync(g => g.Id == id);
            ...
            // Bepaal de unieke monitoren die nog niet aan deze Groepsreis zijn gekoppeld
            ...
            // Bepaal de unieke deelnemers die nog niet aan deze Groepsreis zijn gekoppeld
            var ingeschrevenDeelnemers = groepsreis.Deelnemers.Select(m => m.KindId).ToList();
            var uniekeDeelnemers = deelnemers
                .Where(m => !ingeschrevenDeelnemers.Contains(m.Id))
                .ToList();

            // Ken de beschikbare monitoren en deelnemers toe aan de Groepsreis
            groepsreis.BeschikbareMonitoren = uniekeMonitoren;
            groepsreis.BeschikbareDeelnemers = uniekeDeelnemers;

            // Geef de Groepsreis weer in de view
            return View(groepsreis);
        }
